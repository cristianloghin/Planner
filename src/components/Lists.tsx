...
export function Lists() {
  ...
  return (
    <section className={shared.view}>
      <form className={cx(s.taskAdd, shared.viewHead)} onSubmit={addItem}>
        <input placeholder="Add to a list…" value={title} onChange={(e) => setTitle(e.target.value)} />
            ...
            <li key={t.id} className={s.task}>
              <label>
                <input
                  type="checkbox"
                  checked={t.done}
                  onChange={() => dispatch({ type: 'toggleListItem', id: t.id })}
                />
