 ...
 */
export const Header = createLayout(
  {
    Left: slot(),
    Center: {
      Title: slot(),
      Option: slot({ component: Option, multiple: true }),
    },
    Right: slot(),
  },
  ({ className }: HeaderProps, { slots }) => {
    const { path } = useLocation()
    const { navigate } = useNavigation()
    const options = slots.Center.Option.props.map((p) => ({ to: p.to, label: p.children }))
