...
describe("eventFromDraft", () => {
  ...
  it("writes exactly one recurrence end", () => {
    ...
    expect(eventFromDraft(d).recurrence).toEqual({ freq: "weekly", interval: 2, until: "2026-06-01" });
    expect(eventFromDraft({ ...d, ends: "never" }).recurrence).toEqual({ freq: "weekly", interval: 2 });
  });
...
describe("draftDuration and validity", () => {
  it("never saves something shorter than the snap", () => {
    const d = { ...draftForNew({ date: "2026-04-07", attendees: ["p1"] }), endDT: "2026-04-07T09:05" };
...
describe("templates", () => {
  ...
  it("applying keeps the chosen start and stretches the end", () => {
    ...
    expect(d).toMatchObject({ title: "Dentist", attendees: ["p2"], startDT: "2026-04-07T09:00", endDT: "2026-04-07T09:45" });
